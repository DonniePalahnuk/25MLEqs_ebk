...
{\centering \includegraphics{25MLEqs-V0.0_files/figure-latex/cosine_similarity_example_2-1} 
...
\hypertarget{the-traditional-bayes-formula}{%
\subsection{The traditional Bayes
formula}\label{the-traditional-bayes-formula}}

\textbf{The theorem is usually expressed as:}
...
\tightlist
\item
  \(\operatorname{P}(A\! \mid\! B)\) is the posterior probability. It
  represents the probability of event \(A\) occurring given that \(B\)
  has occurred.
\item
  \(\operatorname{P}(B\! \mid\! A)\) is the likelihood. It represents
  the probability of observing event \(B\) given that \(A\) is true.
\item
  \(\operatorname{P}(A)\) is the prior probability of \(A\). It
  represents the initial belief before considering the new evidence
  \(B\).
\item
  \(\operatorname{P}(B)\) is the marginal probability of \(B\). It
  represents the total probability of observing \(B\) under all possible
  circumstances.
\end{itemize}
...
\tightlist
\item
  Prior Probability \((\operatorname{P}(A))\) : This is your initial
  belief about the probability of an event before any additional
  information is considered. For instance, if you are trying to
...
\item
  Likelihood \((\operatorname{P}(B \mid A)\) ): This reflects how
  probable the evidence \((B)\) is, assuming the hypothesis \((A)\) is
  true. For example, if a diagnostic test is known to detect a disease
  \(95 \%\) of the time when the disease is present, then
  \(\operatorname{P}(B \mid A)=0.95\).
\item
  Marginal Probability \((\operatorname{P}(B))\) : This is the total
  probability of observing the evidence ( \(B\) ) under all possible
  circumstances. It's often calculated as the sum of probabilities for
...
\item
  Posterior Probability \((\operatorname{P}(A \mid B))\) : This is what
  you're solving for. It's your updated belief about the probability of
  \(A\) after taking into account the new evidence \(B\).
\end{enumerate}
...
\textbf{Identities which can by derived by inspection:}
\[\begin{aligned} 
