...
\begin{array}{|c|c|l|}
\hline
\textbf{Term} & \textbf{Category} & \textbf{Description} \\
\hline
\operatorname{P}(A) & \text{Prior Prob} & \text{Initial belief about \(A\) before considering the new evidence \(B\)} \\
\hline
\operatorname{P}(B) & \text{Marginal Prob} & \text{Total probability of observing \(B\) under all possible circumstances} \\
\hline
\operatorname{P}(A\! \mid\! B) & \text{Posterior Prob} & \text{Probability of event \(A\) occurring given that \(B\) has occurred} \\
\hline
\operatorname{P}(B\! \mid\! A) & \text{Likelihood} & \text{Probability of event \(B\) occurring given that \(A\) has occurred} \\
\hline
\end{array}
$$
