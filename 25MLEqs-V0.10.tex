...
\begin{versionhistory}
  ...
  \vhEntry{0.09.0}{2024.11.27}{DP}{Start Linear Regression}
  \vhEntry{0.10.0}{2024.11.27}{DP}{Continue MLE, start OLS}
\end{versionhistory}
\newpage
...
\[
\color{dblu}{\underset{\theta}{\mathop{\mathrm{arg\,max}}} \prod_{i=1}^n P(x_i\! \mid\! \theta)}
\] \normalsize
...
\item
  This model defines how we believe our data is generated
\item
  The vertical bar \textbar{} indicates conditional probability:
...
The product \(\prod\) implies that observations are independent This
allows us to multiply individual probabilities For n observations:
...
\tightlist
\item
  \(\arg\max_{\theta}\) indicates we're searching for the parameter
  value
\item
  This search spans the entire parameter space
\item
  We seek the global maximum of the likelihood function
\end{itemize}

\hypertarget{mathematical-properties}{%
...
\item
  This transforms products into sums while preserving the maximum
\end{itemize}
...
\item
  Second derivatives confirm maximum vs.~minimum
\end{itemize}

\hypertarget{statistical-implications}{%
\subsection{Statistical Implications}\label{statistical-implications}}
...
\item
  Product term grows with each additional observation
\item
...
Maximum Likelihood Estimation (MLE) has its roots in early 20th-century
statistical theory, with its key development attributed to
\textbf{Ronald A. Fisher}. Here's a quick historical account:

\hypertarget{early-foundations}{%
\section{1. Early Foundations}\label{early-foundations}}

\begin{itemize}
\tightlist
\item
  The concept of likelihood dates back to \textbf{Pierre-Simon Laplace}
  (late 18th century), who used a likelihood-based argument for
  parameter estimation in astronomy.
\item
  \textbf{Carl Friedrich Gauss} also employed related concepts in the
  method of least squares for fitting models to data, especially in the
  context of astronomical and geodetic measurements.
\end{itemize}

\hypertarget{fishers-contribution-1920s}{%
\section{2. Fisher's Contribution
(1920s)}\label{fishers-contribution-1920s}}

\begin{itemize}
\tightlist
\item
  The modern form of MLE was developed by \textbf{Ronald A. Fisher} in
  his groundbreaking 1921 paper, \emph{``On the Mathematical Foundations
  of Theoretical Statistics''}. Fisher introduced MLE as a method to
  estimate parameters of a statistical model by maximizing the
  likelihood of observing the data given those parameters.
\item
  Fisher's work formalized MLE and established its theoretical
  underpinnings, including the idea that MLE estimators are
  asymptotically unbiased, efficient, and consistent under regular
  conditions.
\end{itemize}

\hypertarget{impact-and-adoption}{%
\section{3. Impact and Adoption}\label{impact-and-adoption}}
...
\section{4. Further Developments}\label{further-developments}}
...
\hypertarget{mle-introduction}{%
\section{MLE introduction}\label{mle-introduction}}
...
\hypertarget{what-is-maximum-likelihood-estimation}{%
\section{8.1. What is Maximum Likelihood
Estimation?}\label{what-is-maximum-likelihood-estimation}}
...
\subsection{8.1.1. The Likelihood
Function}\label{the-likelihood-function}}

The likelihood function is central to MLE. For a given set of parameters
\(\theta\), it calculates the probability of observing the data \(X\).
For independent and identically distributed (i.i.d.) random variables,
the likelihood is the product of the individual probabilities (or
probability density functions). For example, if the data
...
\section{8.2. Historical Context and
...
Fisher's development of MLE was revolutionary because it provided a
clear, consistent framework for estimating model parameters that was
both flexible and powerful. Over the years, MLE has found widespread use
in many statistical applications, ranging from simple regression models
to complex models in machine learning and bioinformatics.

\hypertarget{mathematical-foundation-of-mle}{%
\section{8.3. Mathematical Foundation of
MLE}\label{mathematical-foundation-of-mle}}

While the concept of MLE is intuitive---finding the parameter values
that make the data most likely---it is grounded in rigorous mathematical
principles. Let's break down the key elements of this process:

\hypertarget{likelihood-function}{%
\subsection{8.3.1. Likelihood Function}\label{likelihood-function}}
...
\[
\mathcal{L}(\theta | X) = P(X_1, X_2, \dots, X_n | \theta)
\]
...
\subsection{8.3.2. The Log-Likelihood}\label{the-log-likelihood}}
...
\subsection{8.3.3. Maximum Likelihood
...
\section{8.4. Connection to Other Estimation
...
\section{8.5. Theoretical Properties of
...
\section{8.6. Conclusion}\label{conclusion}}
...
formulation of MLE, explore examples of its application, and discuss how
to implement MLE in various contexts. We will also examine the
limitations of MLE and compare it to alternative estimation methods.

\newpage

\hypertarget{gpt-dump}{%
\section{GPT Dump}\label{gpt-dump}}

Maximum Likelihood Estimation (MLE) is a statistical method for
estimating the parameters of a probabilistic model. It involves finding
the parameter values that maximize the likelihood of the observed data
under the assumed model. In this chapter, we will discuss the theory
behind MLE, the steps involved in its application, and its connection to
other estimation methods, including least squares estimation.

MLE plays a crucial role in various fields, including regression
analysis, bioinformatics, and machine learning, where it is commonly
used to fit models to data and to make predictions.

\hypertarget{likelihood-function-1}{%
\section{Likelihood Function}\label{likelihood-function-1}}

The \textbf{likelihood function} \(\mathcal{L}(\theta)\) for a given
model is defined as the probability of the observed data as a function
of the model parameters \(\theta\).

\hypertarget{equation-likelihood-function}{%
\subsection{Equation Likelihood
Function}\label{equation-likelihood-function}}
...
\section{Log-Likelihood Function}\label{log-likelihood-function}}

It is often more convenient to work with the \textbf{log-likelihood
function}, as the logarithm simplifies many mathematical operations and
turns products into sums.

\hypertarget{equation-log-likelihood-function}{%
\subsection{Equation Log-Likelihood
Function}\label{equation-log-likelihood-function}}

\[
\ell(\theta) = \log \left( \mathcal{L}(\theta) \right)
\] The log-likelihood is easier to maximize, as it often leads to
simpler differentiation and computation.

\hypertarget{maximum-likelihood-estimator-mle}{%
\section{Maximum Likelihood Estimator
(MLE)}\label{maximum-likelihood-estimator-mle}}

The \textbf{maximum likelihood estimator} \(\hat{\theta}\) is the value
of \(\theta\) that maximizes the log-likelihood function.

\hypertarget{mle-estimator}{%
\subsection{MLE Estimator}\label{mle-estimator}}

\[
\hat{\theta} = \arg\max_{\theta} \ell(\theta)
\] This is the key equation in MLE, where we solve for \(\hat{\theta}\)
...
\] The Fisher Information is used to compute the \textbf{Cramer-Rao
lower bound}, which sets a lower limit on the variance of an unbiased
estimator.
...
\hypertarget{example-8.1-estimating-the-mean-of-a-normal-distribution}{%
\subsection{Example 8.1: Estimating the Mean of a Normal
Distribution}\label{example-8.1-estimating-the-mean-of-a-normal-distribution}}
...
\hypertarget{historical-context-and-development-1}{%
\section{Historical Context and
Development}\label{historical-context-and-development-1}}
...
\hypertarget{the-likelihood-function-1}{%
...
\hypertarget{mle-section-2}{%
\section{MLE Section}\label{mle-section-2}}

\newpage

\hypertarget{mle-section-3}{%
\section{MLE Section}\label{mle-section-3}}

\newpage

\hypertarget{mle-section-4}{%
\section{MLE Section}\label{mle-section-4}}
...
{\centering \includegraphics{25MLEqs-V0.10_files/figure-latex/unnamed-chunk-4-1} 

}

\caption{Default caption for all figures}\label{fig:unnamed-chunk-4}
\end{figure}
\normalsize
...
{\centering \includegraphics{25MLEqs-V0.10_files/figure-latex/unnamed-chunk-5-1} 

}

\caption{Default caption for all figures}\label{fig:unnamed-chunk-5}
\end{figure}
\normalsize
